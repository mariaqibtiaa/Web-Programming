<?php
	if($totalBarang == 0){
		echo "<h3 class='tengah'>Saat ini belum ada data di dalam keranjang belanja anda</h3>";
	}else{
		...
		foreach($keranjang AS $key => $value){
			...
			echo "<tr>
					...
					<td class='tengah'>
					<input type='number' name='$barang_id' value='$quantity' min='1' class='update-quantity'/></td>
					<td class='kanan'>".rupiah($harga)."</td>
					<td class='kanan hapus_item'>".rupiah($total)." <a class='tombol-action' href='".BASE_URL."hapus-item.php?barang_id=$barang_id'>Delete</a></td>
				</tr>";
